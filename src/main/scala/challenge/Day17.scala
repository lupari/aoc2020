...
object Day17 {
  ...
  object Cube {
    def area(bb: (Cube, Cube), hyper: Boolean): Seq[Cube] = {
      ...
      } yield Cube(x, y, z, if (hyper) Option(q) else None)
    }
  }
  case class PocketDim(cubes: Map[Cube, Char]) {
    val hyper: Boolean   = cubes.head._1.q.isDefined
    ...
    def bounds(): (Cube, Cube) =
      (Cube(cubes.keys.minBy(_.x).x,
            ...
            if (hyper) Option(cubes.keys.minBy(_.q).q.get) else None),
       Cube(cubes.keys.maxBy(_.x).x,
            cubes.keys.maxBy(_.y).y,
            cubes.keys.maxBy(_.z).z,
            if (hyper) Option(cubes.keys.maxBy(_.q).q.get) else None))
  }
  ...
  def partTwo(): Int = {
    val pocketDim = PocketDim(
      input.map(kv => (Cube(kv._1.x, kv._1.y, 0, Option(0)), kv._2)).withDefaultValue('.'))
    boot(pocketDim)
  }

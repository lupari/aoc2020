...
object Day02b extends Challenge {

  case class Setting(min: Int, max: Int, letter: Char, pwd: String) {
    def isValid: Boolean = pwd.charAt(min - 1) == letter ^ pwd.charAt(max - 1) == letter
  }
  ...
  override def run(): Any = Source.fromResource("day02.txt").getLines().map(parse).count(_.isValid)
